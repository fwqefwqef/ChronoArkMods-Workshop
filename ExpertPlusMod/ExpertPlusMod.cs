...
namespace ExpertPlusMod
{
    ...
    public class ExpertPlusPlugin : ChronoArkPlugin
    {
        ...
        private static ExpertPlusPlugin instance;
        private static bool isRefreshingSettings;
        ...
        {
            public bool VanillaCurses;
            public bool DespairMode;
            public bool CursedBosses;
            public bool ChaosMode;
            public bool EnableBossChanges;
            public bool ExpertPlusPlus;
        }

        public override void Initialize()
        {
            instance = this;
            ApplySnapshot(ReadConfigSnapshot());
        ...
        private static ConfigSnapshot ReadConfigSnapshot()
        {
            ...
            snapshot.VanillaCurses = modInfo.GetSetting<ToggleSetting>("VanillaCurses").Value;
            snapshot.DespairMode = modInfo.GetSetting<ToggleSetting>("DespairMode").Value;
            snapshot.CursedBosses = modInfo.GetSetting<ToggleSetting>("CursedBosses").Value;
            snapshot.ChaosMode = modInfo.GetSetting<ToggleSetting>("ChaosMode").Value;
            snapshot.EnableBossChanges = modInfo.GetSetting<ToggleSetting>("EnableBossChanges").Value;
            snapshot.ExpertPlusPlus = modInfo.GetSetting<ToggleSetting>("ExpertPlusPlus").Value;

            if (snapshot.ExpertPlusPlus)
            {
                snapshot.VanillaCurses = true;
                snapshot.DespairMode = true;
                snapshot.CursedBosses = true;
                snapshot.ChaosMode = true;
                snapshot.EnableBossChanges = true;
            }

            return snapshot;
        }

        private static bool SnapshotMatchesCurrent(ConfigSnapshot snapshot)
        {
            return VanillaCurses == snapshot.VanillaCurses
                && DespairMode == snapshot.DespairMode
                && CursedBosses == snapshot.CursedBosses
                && ChaosMode == snapshot.ChaosMode
                && EnableBossChanges == snapshot.EnableBossChanges
                && ExpertPlusPlus == snapshot.ExpertPlusPlus;
        }

        private static void ApplySnapshot(ConfigSnapshot snapshot)
        {
            VanillaCurses = snapshot.VanillaCurses;
            DespairMode = snapshot.DespairMode;
            CursedBosses = snapshot.CursedBosses;
            ChaosMode = snapshot.ChaosMode;
            EnableBossChanges = snapshot.EnableBossChanges;
        ...
        private void RebuildPatchState()
        {
            this.harmony.UnpatchSelf();
            PatchNamespace(nameof(ExpertPlusMod));
            if (EnableBossChanges)
            {
                PatchNamespace("BossChanges");
            }
        }

        private static bool RefreshSettingsFromWorkshop()
        {
            ConfigSnapshot snapshot = ReadConfigSnapshot();
            if (SnapshotMatchesCurrent(snapshot))
            {
                return false;
            }
            ...
            if (instance != null)
            {
                instance.RebuildPatchState();
            }

            return true;
        }

        private void PatchNamespace(string namespacePrefix)
        {
            foreach (Type type in typeof(ExpertPlusPlugin).Assembly.GetTypes())
            {
                if (IsPatchTypeInNamespace(type, namespacePrefix))
                {
        ...
        private static bool IsPatchTypeInNamespace(Type type, string namespacePrefix)
        {
            ...
            if (type.Namespace != namespacePrefix && !type.Namespace.StartsWith(namespacePrefix + ".", StringComparison.Ordinal))
            {
                return false;
            }

            if (HasHarmonyAttributes(type))
            {
                return true;
            }
        ...
        private static bool HasHarmonyAttributes(ICustomAttributeProvider provider)
        {
            return provider.GetCustomAttributes(typeof(HarmonyPatch), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyPrefix), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyPostfix), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyTranspiler), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyFinalizer), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyPrepare), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyCleanup), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyTargetMethod), false).Length > 0
                || provider.GetCustomAttributes(typeof(HarmonyTargetMethods), false).Length > 0;
        }

        [HarmonyPatch(typeof(ModManager), nameof(ModManager.SaveModSettings))]
        class RefreshSettingsAfterWorkshopSave
        {
            [HarmonyPostfix]
            static void Postfix(bool SaveFromEntry)
            {
                if (!SaveFromEntry || isRefreshingSettings)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    if (RefreshSettingsFromWorkshop())
                    {
                        Debug.Log("[ExpertPlusMod] Applying workshop config changes.");
                        ModManager.ModStatChanged(true);
                    }
                }
                finally
                {
        ...
        class ModifyGData
        {
            ...
            static void Prefix(ref string dataString)
            {
                if (!_logged)
                {
                    Debug.Log("[ExpertPlusMod] ModifyGData InitFromText prefix active");  
                    _logged = true;
                }
                    ...
                    if (((Dictionary<string, object>)e.Value).ContainsKey("_gdeSchema"))
                    {
                        ...
                        if (e.Key == "Queue_FanaticBoss")
                        {
                            if (DespairMode)
                            {
                                ...
                                // randomize entries in p,a,b
                                List<List<string>> bosses = new List<List<string>>() { p, a, b };
        ...
        class ModifyGData2
        {
            ...
            static void Prefix(ref string dataString)
            {
                    if (!_logged)
                    {
                        Debug.Log("[ExpertPlusMod] ModifyGData2 InitFromText prefix active");
                        _logged = true;
                    }
                    ...
                    foreach (var e in masterJson)
                    {
                        if (((Dictionary<string, object>)e.Value).ContainsKey("_gdeSchema"))
                        {
                            if (((Dictionary<string, object>)e.Value)["_gdeSchema"].Equals("Enemy"))
                            {
                                if (ExpertPlusPlus)
                                {
                                    if ((masterJson[e.Key] as Dictionary<string, object>).ContainsKey("Boss") && Convert.ToBoolean((masterJson[e.Key] as Dictionary<string, object>)["Boss"]))
                                    {
                                    ...
                                    else
                                    {
                                        (masterJson[e.Key] as Dictionary<string, object>)["maxhp"] = (long)((long)(masterJson[e.Key] as Dictionary<string, object>)["maxhp"] * 1.1);
                                        (masterJson[e.Key] as Dictionary<string, object>)["atk"] = (long)((long)(masterJson[e.Key] as Dictionary<string, object>)["atk"] * 1.1);
                                        (masterJson[e.Key] as Dictionary<string, object>)["spd"] = (long)((long)(masterJson[e.Key] as Dictionary<string, object>)["spd"] + 1);
                                    }

                                    (masterJson[e.Key] as Dictionary<string, object>)["reg"] = (long)((long)(masterJson[e.Key] as Dictionary<string, object>)["reg"] * 1.5);
                                    (masterJson[e.Key] as Dictionary<string, object>)["def"] = (long)((long)(masterJson[e.Key] as Dictionary<string, object>)["def"] + 5);
                            }
                            }
                        }
                    }
        ...
        static List<string> tier3 = new List<string>() { "S2_Pierrot_Bat", "S2_DochiDoll", "S2_Horse", "S2_Pierrot_Axe", "S2_Ghost", "S1_Pharos_Warrior", "S2_Pharos_Healer", "S2_Pharos_Mage", "DOWN_MagicDochi", "DOWN_Pharos_HighPriest"};
        static List<string> tier4 = new List<string>() { "S2_Pierrot_Bat", "S2_Horse", "S2_Pierrot_Axe", "S2_Pharos_Mage", "S2_PharosWitch", "S2_Pharos_Warrior", "S2_Pharos_Tanker", "SR_Gunner", "S1_Carpenterdoll", "S3_Wolf", "DOWN_Outlaw" };
        static List<string> tier5 = new List<string>() { "S3_SnowGiant_0", "S3_Pharos_Tanker", "S3_Pharos_HighPriest", "S3_Pharos_Assassin", "S2_Animatronics", "S4_AngryDochi", "UP_LittleMaid", "UP_Pharos_Mage", "UP_Horse", "UP_DochiDoll" };
        static List<string> tier6 = new List<string>() { "S4_MagicDochi", "S4_Summoner", "S1_Armor", "S3_Deathbringer", "S3_Fugitive", "SR_Samurai", "S4_Guard_0_Solo", "S4_Guard_1_Solo", "S4_Guard_2_Solo", "MBoss_0_R", "UP_AngryDochi"};
        ...
        class Curse_Reward_Patch
        {
            ...
            {
                Debug.Log("Speed " + __instance.BChar.GetStat.spd);
                if (!VanillaCurses)
                {
        ...
        // Skill Extended patches: fantasy damage scaling slashed
        [HarmonyPatch(typeof(Skill_Extended), nameof(Skill_Extended.Init))]

        class SkillExtendedPatch
        {
            static void Postfix(Skill_Extended __instance)
            {
                if (__instance is S_LBossFirst_4)
                {
                    //Debug.Log("Damage Scaling slashed");
                    __instance.SkillBasePlus.Target_BaseDMG = -5;
                }
            }
        }
        ...
        class TFKGrave2
        {
            ...
            static void Postfix(BloodyMist __instance)
            {
                if (DespairMode && PlayData.TSavedata.bMist != null && PlayData.TSavedata.bMist.Level == 4 && PlayData.TSavedata.StageNum != 5)
                {
                    ...
                    {
                        BattleSystem.instance.FogTurn = 99;
        ...
        [HarmonyPatch(nameof(Buff.FixedUpdate))]
        class EnsureMaxHP
        {
            [HarmonyPostfix]
            static void Postfix(Buff __instance)
            {
                if (BattleSystem.instance.TurnNum == 0 && __instance.BChar is BattleEnemy)
                {
                    //Debug.Log(__instance.BChar.HP + ", " + __instance.BChar.GetStat.maxhp);
                    if (__instance.BChar.HP != __instance.BChar.GetStat.maxhp)
                    {
                        __instance.BChar.HP = __instance.BChar.GetStat.maxhp;
                    }
                }
            }
        }
