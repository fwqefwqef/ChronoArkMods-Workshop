...
{
    [PluginConfig("Extended Debug Mode", "Extended Debug Mode", "1.0.0")]
    public class DebugModePlugin : ChronoArkPlugin
    {
        ...
        public static KeyCode debugLoadKey = KeyCode.F4;
        public static KeyCode debugSaveKey = KeyCode.F5;
        public static KeyCode reloadGdataKey = KeyCode.F8;

        public static bool enableDebug = true;
        public static bool enableSpeedup = false;
        public static bool rarelearn = false;

        public override void Initialize()
        {
            ModInfo modInfo = ModManager.getModInfo("Extended Debug Mode");
            enableSpeedup = modInfo.GetSetting<ToggleSetting>("enableSpeedup").Value;
        ...
        class UIManager_Patch
        {
            static void Postfix()
            {
                ...
                {
                    if (Input.GetKeyDown(debugLoadKey) && SaveManager.savemanager.DebugMode)
                    {
                        UIManager.InstantiateActive(UIManager.inst.DebugLoad);
                    }
                    if (Input.GetKeyDown(debugSaveKey) && SaveManager.savemanager.DebugMode)
                    {
                        try
                        {
                    ...
                    if (Input.GetKeyDown(reloadGdataKey) && SaveManager.savemanager.DebugMode)
                    {
        ...
        class RareOptions
        {
            ...
            static bool Prefix(UseItem.SkillBookCharacter_Rare __instance, ref bool __result)
            {
                if (rarelearn)
                {
                    ...
                    Debug.Log("Rare Skills infinity");
